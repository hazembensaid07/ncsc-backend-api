...
exports.addbooking = async (req, res) => {
  try {
    //update booked attribute for all the roomates and add booking document in booking collection
    const { room } = req.body;
    id_maker = req.user?._id;
    let phones = [];
    let emails = [];
    const result = await User.findById(id_maker)
      .populate("roomMates")
      .select("email,phone");

    result.roomMates.map(async (el) => {
      phones.push(el.phone);
...
exports.getAvailableRooms_Transport = async (req, res) => {
  try {
    ...
    res.status(200).send({
      ...
      triple_room: triple,
      message: "rooms per category",
      rest_triple_rooms: resu[0].triple_rooms,
      rest_persons: resu[0].rooms,
    });
  } catch (error) {
  ...
  try {
    ...
    if (req.user.roomMates.length == 2) {
      const psu = await Hotel.updateOne(
        ...
        {
          $set: {
            rooms: (r[0].rooms += req.user.roomMates.length + 1),
...
exports.deleteRoomMate = async (req, res) => {
  try {
    if (req.user.booking) {
      res.status(400).send({ error: "can not delete , cancel booking first" });
    } else {
      const { id } = req.body;
